...
function busterFunctionalMixin(testCase, options)
{
  ...
  //swipe right or left distance
  testCase._swipeRightOrLeftDistance = options.swipe_right_or_left_distance || 200;

  //swipe up or down distance
  testCase._swipeUpOrDownDistance = options.swipe_up_or_down_distance || 350;

  //swipe right or left distance
  testCase._swipeRightOrLeftOffsetFactor = options.swipe_right_or_left_offset_percentage || 0.7;

  //swipe up or down distance
  testCase._swipeUpOrDownOffsetFactor = options.swipe_up_or_down_offset_percentage || 0.45;
  ...
  testCase.swipeLeft = function busterFunctionalMixin_swipeLeft(target, callback)
  {
    // update offset value
    var offset = this.window && this.window.innerWidth ?
                 this.window.innerWidth * this._swipeRightOrLeftOffsetFactor :
                 this._swipeRightOrLeftDistance;

    this._swipe(target, [-Math.abs(offset), 0], callback);

    return this;

  };

  testCase.swipeRight = function busterFunctionalMixin_swipeRight(target, callback)
  {
    // update offset value
    var offset = this.window && this.window.innerWidth ?
                 this.window.innerWidth * this._swipeRightOrLeftOffsetFactor :
  ...
  testCase.swipeUp = function busterFunctionalMixin_swipeUp(target, callback)
  {
    // update offset value
    var offset = this.window && this.window.innerHeight ?
                 this.window.innerHeight * this._swipeUpOrDownOffsetFactor :
                 this._swipeUpOrDownDistance;

    this._swipe(target, [0, Math.abs(offset)], callback);

    return this;

  };

  testCase.swipeDown = function busterFunctionalMixin_swipeDown(target, callback)
  {
    // update offset value
    var offset = this.window && this.window.innerHeight ?
                 this.window.innerHeight * this._swipeUpOrDownOffsetFactor :
  ...
  // Waits for target element to be not empty
  testCase.waitForElementToContain = function busterFunctionalMixin_waitForElementToContain(target, callback)
  {
    target = typeof target == 'string' ? this.$(target) : target;

    // check if the target element is present & and it's not empty
    if (target.length && !target.is(':empty'))
    {
      // and it's async
      this._delayedCallback(callback);
      return this;
    ...
    // no luck, try later
    setTimeout(this.waitForElementToContain.bind(this, target, callback), this._cooldownPeriod);
  ...
  testCase._swipe = function busterFunctionalMixin_swipe(target, swipeCoordinates, callback)
  {
    // get first element of the selection
    target = (typeof target == 'string' ? this.$(target) : target).first();
    ...
    setTimeout(function()
    {
      ...
      // as second dimension array
      // and pass extra arguments for createEvent function
      this._triggerEvents(target, ['touchstart', ['touchmove', 'touchend']], target, swipeCoordinates, callback);
  ...
  testCase._createEvent = function busterFunctionalMixin__createEvent(type, extra, eventCoordinates)
  {
    var handler;
  ...
  // Creates touch event like real boyz (browsers) do
  testCase._createTouchEvent = function busterFunctionalMixin__createTouchEvent(type, target, eventCoordinates)
  {
    var eventData  = {}
      , targetX    = target.offset().left + (target.width()/2)
      , targetY    = target.offset().top + (target.height()/2)
      , XOffset    = eventCoordinates ? eventCoordinates[0] : 0
      , YOffset    = eventCoordinates ? eventCoordinates[1] : 0
      , touchcoordinates = {}
      , touchEvent =
      {
        type: type,
    ...
    // event specific data
      ...
      touchMove =
      {
        touches:
          [
            {clientX: targetX + XOffset, clientY: targetY + YOffset, pageX: targetX + XOffset, pageY: targetY + YOffset}
          ]
      }
      ;
    ...
    {
      case 'touchstart':
        touchcoordinates = touchStart;
        break;
      case 'touchmove':
        touchcoordinates = touchMove;
        break;
    }

    // untangle result event object
    eventData = this.$.extend(true, {}, touchEvent, touchcoordinates);
